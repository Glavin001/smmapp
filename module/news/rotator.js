...
// Start the news feed rotator
    ...
    $('#newsFeed ul.newsList li.newsItem img').on('dragstart', function(event) {
        event.preventDefault();
    }); // Disable default action for dragging image
    $(document).on('vmousedown', "#newsFeed ul.newsList li.newsItem", function(clickEvent) {
        ...
        startPosition = {x: clickEvent.pageX, y: clickEvent.pageY };
        console.log("Starting position:" + startPosition);
        $("#newsFeed ul.newsList li.newsItem").stop();

        $(document).on('vmousemove', function(moveEvent) {
            var newPos = {x: moveEvent.pageX, y: moveEvent.pageY };
            //console.log(newX);
            //console.log(startPosition.x - newX);
            var newRotatePosition = rotatePosition + (startPosition.x - newPos.x);
            ...
            if (scrollPrevented == false) {
                ...
                $(document).on('touchmove', function(ev) {
                    console.log("bet you can't scroll!");
                    if (scrollPrevented == true)
                        ev.preventDefault();
                });
            }
            
           $('body').scrollTop(  $('body').scrollTop() + (startPosition.y-newPos.y) );
        ...
        $(document).on('vmouseup', function(upEvent) {
            ...
            // Find closest
            var newPos = {x: upEvent.pageX, y: upEvent.pageY};
