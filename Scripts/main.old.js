...
window.addEventListener("orientationchange", function() { hideAddressBar(); loadAppGrid(); } )
window.addEventListener("resize", function() { loadAppGrid(); } )
...
$(document).on('vmousedown', 'div.smuToolsPanel', function(event) {
    //alert("vmousedown");
...
function loadAppGrid()
{
  ...
  max_apps_row = Math.floor(grid_view.width() / app_width);
